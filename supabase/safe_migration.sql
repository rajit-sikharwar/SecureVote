-- Safe Migration: Update existing schema to student-focused system
-- This version handles existing objects and policies gracefully
...
-- Step 8: Create votes table (if not exists)
CREATE TABLE IF NOT EXISTS public.votes (
    ...
    UNIQUE(voter_id, election_id) -- One vote per voter per election
);
...
BEGIN
    ...
    WHERE voter_id = p_user_id AND election_id = p_election_id;
    ...
    -- Cast vote
    INSERT INTO public.votes (voter_id, election_id, candidate_id, receipt_hash)
...
BEGIN
    ...
    CREATE POLICY "Users can view their votes" ON public.votes FOR SELECT USING (auth.uid() = voter_id);
