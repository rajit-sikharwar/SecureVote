...
interface CandidateCardProps {
  candidate: Candidate;
  totalVotes?: number;
  voteCount?: number; // Additional prop for vote count when available
  hasVoted?: boolean;
  isWinner?: boolean;
  onVoteClick?: () => void;
  onViewBio?: () => void;
...
export function CandidateCard({
  candidate,
  totalVotes = 0,
  voteCount = 0, // Use passed voteCount prop, default to 0
  hasVoted,
  isWinner,
  onVoteClick,
  onViewBio
    ...
    <Card className="p-4 flex flex-col h-full border-2 border-transparent transition-colors hover:border-gray-100">
      <div className="flex gap-4 items-start mb-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-sm text-gray-500 truncate">{department}</p>
        </div>
      </div>
